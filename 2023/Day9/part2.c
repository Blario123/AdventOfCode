...
void parseLine(const char* l, size_t length) {
    ...
    tempSeq.firstnumbers = malloc((tempSeq.numbersSize - 1) * sizeof(int));
    memset(tempSeq.firstnumbers, 0, (tempSeq.numbersSize - 1) * sizeof(int));
    
    int* subsequence = malloc(tempSeq.numbersSize * sizeof(int));
    memcpy(subsequence, tempSeq.numbers, tempSeq.numbersSize * sizeof(int));
    
    tempSeq.firstnumbers[depth++] = tempSeq.numbers[0];
    ...
    while(!allZeros) {
        ...
        for(int j = 1; j < tempSize; j++) {
            ...
            temp[j-1] = delta;
            if(j == tempSize - 1) {
                tempSeq.firstnumbers[depth++] = temp[0];
            }
        }
        tempSize--;
    ...
    for(int j = depth - 1; j >= 0; j--) {
        carry = tempSeq.firstnumbers[j] - carry;
    }
    score += carry;
    free(tempSeq.numbers);
    free(subsequence);
...
int main(int argc, char** argv) {
    if(argc > 1) {
        ...
        printf("Total sum of the histories = %lld\n", score);
    } else {
